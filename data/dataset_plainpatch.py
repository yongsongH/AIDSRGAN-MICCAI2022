...
class DatasetPlainPatch(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        # ------------------------------------
        temp = self.patch_size
        self.total_patches = self.num_sampled * self.num_patches_per_image
        self.H_data = np.zeros([self.total_patches, temp, temp, self.n_channels], dtype=np.uint8)
        self.L_data = np.zeros([self.total_patches, temp, temp, self.n_channels], dtype=np.uint8)
    ...
    def get_patches(self, index):
        ...
        i = self.patch_size
        for _ in range(num):
            rnd_h = random.randint(0, max(0, H - i))
            rnd_w = random.randint(0, max(0, W - i))
            L_patch = img_L[rnd_h:rnd_h + i, rnd_w:rnd_w + i, :]
            H_patch = img_H[rnd_h:rnd_h + i, rnd_w:rnd_w + i, :]
