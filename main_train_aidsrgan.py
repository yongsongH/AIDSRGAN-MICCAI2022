...
from models import enhance_sr_denoising as net


def main(json_path='options/train_enhance_msrresnet_denosing_add_deloss.json'):
    '''
    # ----------------------------------------
    # Step--1 (prepare opt)
    # ----------------------------------------
    ...
    G_state_dict = torch.load('superresolution/train_deloss_l1__RCABlock_x2_0209/models/130000_G.pth')
    D_state_dict = torch.load('superresolution/train_deloss_l1__RCABlock_x2_0209/models/130000_D.pth')


    from collections import OrderedDict
    # #
    G_new_state_dict = OrderedDict() # 2021/12/16
    ...
    D_new_state_dict = OrderedDict()  # 2021/12/16

    for k, v in D_state_dict.items():
        name = str('module.' + k[0:])
        D_new_state_dict[name] = v
        model.netD.load_state_dict(D_new_state_dict, strict=False)

    print('Loading model for G and D ...')
    ...
    for epoch in range(70000):  # keep running
        for i, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['checkpoint_test'] == 0:
                ...
                # testing log
                logger.info(
                    '<epoch:{:3d}, iter:{:8,d}, Average PSNR : {:<.2f}dB, Average PSNR : {:<.4f}\n'.format(epoch,
                                                                                                           current_step,
                                                                                                           avg_psnr,
                                                                                                           avg_ssim))
