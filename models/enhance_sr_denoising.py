...
from models.loss import VGGFeatureExtractor
...
class ModelGAN(ModelBase):
    ...
    def define_loss(self):
        ...
        if self.opt_train['G_lossfn_weight'] > 0:
            ...
            # denoising loss
            G_denoisingloss_type = self.opt_train['G_denoisingloss_type']
            if G_denoisingloss_type == 'l1':
                self.G_denoisingloss_type = nn.L1Loss().to(self.device)
            elif G_denoisingloss_type == 'l2':
                self.G_denoisingloss_type = nn.MSELoss().to(self.device)
            elif G_denoisingloss_type == 'l2sum':
                self.G_denoisingloss_type = nn.MSELoss(reduction='sum').to(self.device)
            elif G_lossfn_type == 'ssim':
                self.G_denoisingloss_type = SSIMLoss().to(self.device)
            if G_lossfn_type == 'l1':
            ...
            self.G_srlossfn_weight = self.opt_train['G_srlossfn_weight']
            self.G_delossfn_weight = self.opt_train['G_delossfn_weight']
        else:
    ...
    def feed_data(self, data, need_H=True):
        ...
        self.LHR = data['LHR'].to(self.device)
        if need_H:
    ...
    def optimize_parameters(self, current_step):
        ...
        self.E, self.DE = self.netG(self.L) # 2021/12/04 添加loss

        loss_G_total = 0

        if current_step % self.D_update_ratio == 0 and current_step > self.D_init_iters:  # updata D first
            if self.opt_train['G_lossfn_weight'] > 0:
                G_loss = self.G_srlossfn_weight * self.G_lossfn(self.E, self.H) +\
                         self.G_delossfn_weight * self.G_lossfn(self.DE, self.LHR)# 2021/12/04 添加loss
    ...
    def test(self):
        self.netG.eval()
        with torch.no_grad():
            self.E, self.LSR = self.netG(self.L)
        self.netG.train()
